...
# 砖块生成配置（行列数由 SettingsManager 动态读取）
...
const GAME_WIDTH: float = 800.0     # 游戏区域宽度（用于居中计算）
...
func reset_game() -> void:
	score = 0
	lives = SettingsManager.get_setting("initial_lives", 3)
	is_game_over = false
	is_paused = false
	# 状态变更后通知 UI 更新
...
## 生成砖块网格（由主场景调用，行列数从 SettingsManager 读取）
func spawn_bricks(container: Node2D) -> void:
	var brick_cols: int = SettingsManager.get_brick_cols()
	var brick_rows: int = SettingsManager.get_brick_rows()
	var x_step: float = BRICK_WIDTH + BRICK_GAP
	var y_step: float = BRICK_HEIGHT + BRICK_GAP
	# 动态计算起始X坐标，使砖块在屏幕水平居中
	var total_width: float = brick_cols * x_step - BRICK_GAP
	var start_x: float = (GAME_WIDTH - total_width) / 2.0
